...
zigbee_throughout_no_interference_50_25ms = 16.00 .* ones(6,1);
...
zigbee100byte25ms = t;
load('.\ZigbeeThroughput\zigbee50byte25ms.mat')
zigbee50byte25ms = t;
...
figure,
plot(1000./wifi_delay, zigbee100byte25ms, 'bo-', ...
    1000./wifi_delay, zigbee_throughput_no_interference_100_25ms, 'r--')
...
packetsize = [10,50,100]';
zigbee25ms = [zigbee10byte25ms; zigbee50byte25ms; zigbee100byte25ms];
figure,
hold on
for i = 1:6
...
figure,
semilogy(wifi_delay, zigbee10byte10ms, 'bo-', ...
    wifi_delay, zigbee100byte100ms, 'ro--')

hold on,
semilogy(wifi_delay, zigbee50byte25ms, 'bs-', ...
    wifi_delay, zigbee100byte50ms, 'rs--')
hold off

xlabel('wifi constant delay (ms)')
ylabel('zigbee throughput (kbps)')
legend('ZigBee: 10 byte / 10ms',...
    'ZigBee: 100 byte / 100ms', ...
    'ZigBee: 50 byte / 25ms', ...
    'ZigBee: 100 byte / 50ms')
title('ZigBee throughput under WiFi512 interference')
grid on
